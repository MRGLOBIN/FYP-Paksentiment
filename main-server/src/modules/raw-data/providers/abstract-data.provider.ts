  ...
  protected async saveSession(
    ...
    if (existingSession) {
      ...
      if (newPostIds.length > 0) {
        ...
        this.logger.log(
          `Appended ${newPostIds.length} posts to existing session ${sessionId}`,
        );
      }
    } else {
      ...
      this.logger.log(
        `Saved new session ${sessionId} with ${postIds.length} posts for user ${userId}`,
      );
    }
  }
