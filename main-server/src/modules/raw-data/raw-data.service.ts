  ...
  private async saveSessionLocal(
    ...
    if (existingSession) {
      const newPostIds = postIds.filter(
        (id) => !existingSession.postIds.includes(id),
      );
...
`,
  ...
  async selectAILinks(userQuery: string): Promise<any> {
    ...
    try {
      ...
      this.logger.error(
        '[SelectAILinks] Could not read trusted_links.json, falling back to defaults:',
        err.message,
      );
      categorizedLinks = {
        Business: [
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an intelligent routing and categorization assistant for a news aggregation platform. Respond ONLY with valid JSON arrays.',
        },
        {
          role: 'user',
...
Example format: ["https://example.com/target-1", "https://example.com/target-2"]`,
    ...
    try {
      ...
      let selectedUrls = parsed.filter(
        (url) => typeof url === 'string' && allTrustedLinks.includes(url),
      );
      ...
      throw new InternalServerErrorException(
        'Failed to get AI link suggestions',
      );
    }
  }
