...
export class CrawlerController {
  ...
  @ApiOperation({
    ...
    description:
      'Fetches a single page using the high-performance Go Colly sidecar. Best for static HTML pages.',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiOperation({
    ...
    description:
      'Performs a multi-page deep crawl using Go Colly with configurable depth, limits, and domain restrictions.',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiResponse({
    status: 202,
    description:
      'Crawl job started securely in the background. Polling session UUID returned.',
  })
  async crawl(
    @Body('url') url: string,
    ...
    this.logger.log(
      `[Crawl] Request for ${url} depth=${maxDepth} limit=${limit}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Uses the Python FastAPI/Scrapling backend for sites that require JavaScript rendering.',
  })
  @ApiBody({
    schema: {
      type: 'object',
