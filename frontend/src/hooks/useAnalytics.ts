    ...
    async (overrideSessionId?: string) => {
      ...
      try {
        if (activeSessionId) {
          ...
          if (urls.length === 0)
            throw new Error(
              "No valid URLs provided. Each URL must start with http:// or https://",
            );
          ...
          for (let i = 0; i < urls.length; i++) {
            setStatusMsg(
              `Processing URL ${i + 1}/${urls.length}: ${urls[i].substring(0, 50)}...`,
            );
            try {
          ...
          if (!aiRes.ok)
            throw new Error("AI could not select sources based on the query.");
          const aiData = await aiRes.json();
          const aiUrls: string[] = aiData.urls || [];

          if (aiUrls.length === 0)
            throw new Error("AI returned no valid URLs.");
          ...
          for (let i = 0; i < aiUrls.length; i++) {
            setStatusMsg(
              `AI Selected ${aiUrls.length} links. Processing ${i + 1}/${aiUrls.length}: ${aiUrls[i].substring(0, 50)}...`,
            );
            try {
              const result = await fetchSource(
                apiUrl,
              ...
              console.warn(
                `Failed to process AI selected ${aiUrls[i]}: ${urlErr.message}`,
              );
            }
          }
