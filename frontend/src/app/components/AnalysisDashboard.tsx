...
export default function AnalysisDashboard({ data }: AnalysisDashboardProps) {
    ...
    return (
      data.posts?.map((post, idx) => {
        ...
        return {
          ...
          Source: post.url?.includes("reddit")
            ? "Reddit"
            : post.url?.includes("youtube")
              ? "YouTube"
              : post.url?.includes("twitter")
                ? "Twitter"
                : "Web",
          ...
          Confidence:
            ((post.confidence || sentiment?.confidence || 0) * 100).toFixed(1) +
            "%",
          URL: post.url || "N/A",
        };
      }) || []
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `paksentiment_analysis_${new Date().toISOString().slice(0, 10)}.csv`,
    );
  ...
  const handleExportPDF = () => {
    ...
    doc.text(
      `Source: ${data.source?.toUpperCase()} | Documents: ${data.count}`,
      14,
      36,
    );
    ...
    doc.save(
      `paksentiment_report_${new Date().toISOString().slice(0, 10)}.pdf`,
    );
    setExportAnchorEl(null);
  };
  ...
  const kpis = useMemo(() => {
    ...
    const uniqueAuthors = new Set(
      data.posts?.map((p) => p.author).filter(Boolean),
    ).size;
    ...
    const topicCounts = sentimentSource.reduce(
      (acc, curr) => {
    ...
    const avgConfidence =
      sentimentSource.length > 0
        ? sentimentSource.reduce((sum, s) => sum + (s.confidence || 0), 0) /
          sentimentSource.length
        : 0;
    ...
    return {
      ...
      topTopicPercent:
        topTopic && totalDocs > 0
          ? ((topTopic[1] / totalDocs) * 100).toFixed(1)
          : "0",
  ...
  const sourceData = useMemo(() => {
    ...
    data.posts?.forEach((p) => {
      ...
      else if (p.author?.includes("."))
        source = p.author.split(".")[0]; // Domain
            ...
            <div className={styles.kpiCard}>
              ...
              <div className={styles.kpiContent}>
                <span
                  ...
                  style={{
                    color:
                      TOPIC_COLORS[kpis.topTopic.toLowerCase()] ||
                      COLORS.primary,
                  }}
                >
                  {kpis.topTopic}
                ...
                <span className={styles.kpiTitle}>
                  Top Sentiment ({kpis.topTopicPercent}%)
                </span>
              </div>
            </div>
            ...
            data.posts?.map((post, idx) => {
              ...
              return {
                ...
                summary:
                  sentiment?.summary ||
                  post.title ||
                  (post.text || post.content || "").substring(0, 200) + "...",
          ...
          columns={[
            ...
            },
            {
              ...
              renderCell: (params) => {
                ...
                const color = val.includes("positive")
                  ? "#10b981"
...

            {
              ...
              renderCell: (params) => (
                <Button
                  ...
                  size="small"
                  onClick={() => {
                    const original = data.posts?.find(
                      (p, i) => (p.id || i) === params.id,
                    );
            ...
            <DialogContent dividers>
              <Stack spacing={2}>
                ...
                <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                  <Chip
                    label={`Author: ${selectedPost.author || "Unknown"}`}
                    variant="outlined"
                  />
                  {selectedPost.sentiment && (
                    <Chip
                      ...
                      sx={{
                        backgroundColor:
                          TOPIC_COLORS[selectedPost.sentiment.toLowerCase()] ||
                          COLORS.neutral,
                        color: "#fff",
                      }}
                    />
                  ...
                  {selectedPost.confidence && (
                    <Chip
                      label={`Confidence: ${(selectedPost.confidence * 100).toFixed(1)}%`}
                      variant="outlined"
                    />
                  )}
                  {selectedPost.timestamp && (
                    <Chip
                      label={`Date: ${new Date(selectedPost.timestamp).toLocaleString()}`}
                      variant="outlined"
                    />
                  )}
                </Box>
